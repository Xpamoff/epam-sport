...
if(isset($_COOKIE['id'])and isset($_COOKIE['ride_total'])and isset($_COOKIE['run_total'])and isset($_COOKIE['swim_total'])and isset($_COOKIE['name'])){
    ...
    if(!empty($check)){
        mysqli_query($link, "UPDATE `user-data` SET `id`='".$_COOKIE['id']."',`name`='".rawurldecode($_COOKIE['name'])."',`rides-total`='".$_COOKIE['ride_total']."',`swim-total`='".$_COOKIE['swim_total']."',`run-total`='".$_COOKIE['run_total']."' WHERE id=".$_COOKIE['id']);
    }
    else{
